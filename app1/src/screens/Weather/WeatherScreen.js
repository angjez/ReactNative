...
const findMinMax = (arr, min) => {
  ...
  for (var i = 0; i < arr.length; i++) {
    min
      ? tempArr.push(arr[i].main.temp_min)
      : tempArr.push(arr[i].main.temp_max);
  }
...
const WeatherScreen = () => {
  ...
  useEffect(() => {
    Geolocation.getCurrentPosition((info) => {
      fetch(
        `https://api.openweathermap.org/data/2.5/forecast?lat=${info.coords.latitude}&lon=${info.coords.longitude}&exclude={part}&appid=${key}`,
      )
        .then((res) => res.json())
        .then((json) => {
    ...
    <LinearGradient
      ...
      <View style={styles.viewStyle}>
        ...
        <Text style={styles.descriptionTextStyle}>
          {todaysWeather[0] ? todaysWeather[0].weather[0].main : 'Loading'}
        </Text>
        ...
        <FlatList
          ...
          scrollEnabled="true"
          keyExtractor={() => todaysWeather.dt_txt}
          data={todaysWeather}
          renderItem={({item}) => {
            return (
              <View style={styles.flatlistItemViewStyle}>
                <Text style={styles.flatListTextStyle}>
                  {item.dt_txt[11] === 0
                    ? item.dt_txt.substring(12, 13)
                    : item.dt_txt.substring(11, 13)}
                </Text>
                <Icon
                  name={chooseIcon(item.weather[0].main)}
                  size={15}
                  color="white"
                />
                <Text style={styles.flatListTextStyle}>{`${Math.round(
              ...
              return (
                <>
                  ...
                  <View style={styles.forecastFlatlistItemViewStyle}>
                    ...
                    <Text
                      style={
                        styles.forecastFlatlistTextStyle
                      }>{`Min :${Math.round(
                      findMinMax(forecast.slice(index, index + 7), true) -
                        273.15,
                    )}°C`}</Text>
                    <Text
                      style={
                      ...
                      findMinMax(forecast.slice(index, index + 7), false) -
                        273.15,
                    )}°C`}</Text>
                  </View>
                </>
